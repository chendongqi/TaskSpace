...
export class DataStorage {
  ...
  async initializeStorage(options = {}) {
    ...
    try {
      ...
      if (options.mergeAnonymousData) {
        ...
        // ✅ 修复：优化同步逻辑，处理首次登录场景
        // ✅ 获取上次同步时间戳，用于判断本地任务是新增还是被其他设备删除
        ...
        for (const key of keys) {
          ...
          if (serverData && serverData.data) {
            // ✅ 关键修复：检查本地数据是否为空
            const isLocalDataEmpty = this.isDataEmpty(localData);

            let finalData;
            if (isLocalDataEmpty) {
              // 本地数据为空（首次登录或清空后），直接使用服务器数据
              finalData = serverData.data;
              console.log(`📥 Local data is empty, using server data for ${key}`);
            } else {
              // 本地数据不为空，进行时间戳比较和合并
              const hasRecentLocalChanges = await this.hasRecentLocalChanges(key, serverData.timestamp);

              if (hasRecentLocalChanges) {
                // 本地有更新的修改，合并数据（传入 lastSyncTimestamp 用于识别跨设备删除）
                finalData = await this.mergeData(key, localData, serverData.data, lastSyncTimestamp);
                console.log(`🔀 Merged local and server data for ${key}`);
              } else {
                // 服务器数据更新，直接使用
                finalData = serverData.data;
  ...
  // 智能数据合并
  // ✅ 新增 lastSyncTimestamp 参数，用于判断本地任务是新增还是已被其他设备删除
  async mergeData(key, localData, serverData, lastSyncTimestamp = null) {
    if (!serverData) return localData;
    if (!localData) return serverData;

    switch (key) {
      case 'dailyTasks':
        return this.mergeDailyTasks(localData, serverData, lastSyncTimestamp);
      case 'backlogTasks':
        return this.mergeBacklogTasks(localData, serverData, lastSyncTimestamp);
      case 'habits':
        return this.mergeHabits(localData, serverData, lastSyncTimestamp);
      case 'customTags':
        return this.mergeCustomTags(localData, serverData);
      case 'yearlyGoals':
        return this.mergeYearlyGoals(localData, serverData, lastSyncTimestamp);
      case 'quarterlyGoals':
        return this.mergeQuarterlyGoals(localData, serverData, lastSyncTimestamp);
      case 'weeklyGoals':
        return this.mergeWeeklyGoals(localData, serverData, lastSyncTimestamp);
      case 'darkMode':
      case 'theme':
        // 简单值，以服务器数据为准
        return serverData;
  ...
  // 合并日常任务
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeDailyTasks(localTasks, serverTasks, lastSyncTimestamp = null) {
    if (!localTasks && !serverTasks) return {};
    if (!localTasks) return serverTasks;
    if (!serverTasks) return localTasks;

    const merged = { ...serverTasks };
    const syncTime = lastSyncTimestamp ? new Date(lastSyncTimestamp).getTime() : 0;

    Object.keys(localTasks || {}).forEach(dateKey => {
      if (!merged[dateKey]) {
        // ✅ 修复：整天的任务都需要检查是本地新增还是被删除
        const validTasks = localTasks[dateKey].filter(task => {
          const taskCreatedAt = new Date(task.createdAt).getTime();
          // 如果任务创建时间 > 上次同步时间，说明是同步后新建的
          if (!lastSyncTimestamp || taskCreatedAt > syncTime) {
            console.log(`➕ Keeping new local task: ${task.title}`);
            return true;
          } else {
            // 任务在同步时就存在，但服务器现在没有 → 被其他设备删除
            console.log(`🗑️ Task "${task.title}" was deleted on another device`);
            return false;
          }
        });
        if (validTasks.length > 0) {
          merged[dateKey] = validTasks;
          console.log(`📅 Adding ${validTasks.length} local tasks for date ${dateKey}`);
        }
      } else {
        // 合并同一天的任务
        ...
        localDayTasks.forEach(localTask => {
          ...
          if (existingIndex === -1) {
            // ✅ 关键修复：检查是本地新增还是被删除
            const taskCreatedAt = new Date(localTask.createdAt).getTime();
            if (!lastSyncTimestamp || taskCreatedAt > syncTime) {
              mergedDayTasks.push(localTask);
              console.log(`➕ Adding new local task: ${localTask.title}`);
            } else {
              console.log(`🗑️ Task "${localTask.title}" was deleted on another device`);
            }
          } else {
            // 存在冲突，比较时间戳和内容
  ...
  // 合并 Backlog 任务
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeBacklogTasks(localTasks, serverTasks, lastSyncTimestamp = null) {
    if (!localTasks && !serverTasks) return [];
    if (!localTasks) return serverTasks;
    if (!serverTasks) return localTasks;
    ...
    localTasks.forEach(task => {
      if (taskMap.has(task.id)) {
        ...
        // ✅ 关键修复：本地有、服务器没有 → 检查是新增还是被删除
        const taskCreatedAt = new Date(task.createdAt).getTime();
        if (!lastSyncTimestamp || taskCreatedAt > syncTime) {
          // 同步后新建的任务，保留
          taskMap.set(task.id, task);
          console.log(`➕ Keeping new local backlog task: ${task.title}`);
        } else {
          // 任务在同步时就存在，但服务器没有 → 被其他设备删除
          console.log(`🗑️ Backlog task "${task.title}" was deleted on another device`);
        }
      }
    });
  ...
  // 合并习惯
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeHabits(localHabits, serverHabits, lastSyncTimestamp = null) {
    ...
    localHabits.forEach(localHabit => {
      // 习惯通过 id 来匹配（如果有），否则通过名称
      const existingIndex = merged.findIndex(h =>
        (h.id && localHabit.id && h.id === localHabit.id) ||
        h.name.toLowerCase() === localHabit.name.toLowerCase()
      );

      if (existingIndex === -1) {
        // ✅ 关键修复：本地有、服务器没有 → 检查是新增还是被删除
        const habitCreatedAt = localHabit.createdAt ? new Date(localHabit.createdAt).getTime() : 0;
        if (!lastSyncTimestamp || habitCreatedAt > syncTime) {
          // 同步后新建的习惯，保留
          merged.push(localHabit);
          console.log(`🎯 Keeping new local habit: ${localHabit.name}`);
        } else {
          // 习惯在同步时就存在，但服务器没有 → 被其他设备删除
          console.log(`🗑️ Habit "${localHabit.name}" was deleted on another device`);
        }
      } else {
        // 合并完成日期
  ...
  // 合并年度目标
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeYearlyGoals(localGoals, serverGoals, lastSyncTimestamp = null) {
    ...
    localGoals.forEach(localGoal => {
      ...
      if (existingIndex === -1) {
        // ✅ 关键修复：本地有、服务器没有 → 检查是新增还是被删除
        const goalCreatedAt = new Date(localGoal.createdAt).getTime();
        if (!lastSyncTimestamp || goalCreatedAt > syncTime) {
          // 同步后新建的目标，保留
          merged.push(localGoal);
          console.log(`🎯 Keeping new local yearly goal: ${localGoal.title}`);
        } else {
          // 目标在同步时就存在，但服务器没有 → 被其他设备删除
          console.log(`🗑️ Yearly goal "${localGoal.title}" was deleted on another device`);
        }
      } else {
        // 存在冲突，比较进度和更新时间
        const existing = merged[existingIndex];
        ...
        if (localProgress > existingProgress ||
          ...
          console.log(`🔄 Updated yearly goal with local version: ${localGoal.title}`);
        }
      }
    });
  ...
  // 合并季度目标
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeQuarterlyGoals(localGoals, serverGoals, lastSyncTimestamp = null) {
    if (!localGoals && !serverGoals) return [];
    if (!localGoals) return serverGoals;
    if (!serverGoals) return localGoals;
    ...
    localGoals.forEach(localGoal => {
      ...
      if (existingIndex === -1) {
        ...
        if (!lastSyncTimestamp || goalCreatedAt > syncTime) {
          // 同步后新建的目标，保留
          merged.push(localGoal);
          console.log(`📊 Keeping new local quarterly goal: ${localGoal.title}`);
        } else {
          // 目标在同步时就存在，但服务器没有 → 被其他设备删除
          console.log(`🗑️ Quarterly goal "${localGoal.title}" was deleted on another device`);
        }
      } else {
        // 存在冲突，比较进度和更新时间
        const existing = merged[existingIndex];
  ...
  // 合并周目标
  // ✅ 新增 lastSyncTimestamp 参数，用于识别跨设备删除
  mergeWeeklyGoals(localGoals, serverGoals, lastSyncTimestamp = null) {
    if (!localGoals && !serverGoals) return [];
    if (!localGoals) return serverGoals;
    if (!serverGoals) return localGoals;
    ...
    localGoals.forEach(localGoal => {
      ...
      if (existingIndex === -1) {
        ...
        if (!lastSyncTimestamp || goalCreatedAt > syncTime) {
          // 同步后新建的目标，保留
          merged.push(localGoal);
          console.log(`📅 Keeping new local weekly goal: ${localGoal.title}`);
        } else {
          // 目标在同步时就存在，但服务器没有 → 被其他设备删除
          console.log(`🗑️ Weekly goal "${localGoal.title}" was deleted on another device`);
        }
      } else {
        // 存在冲突，比较进度和更新时间
        const existing = merged[existingIndex];
  ...
  // ✅ 新增：立即备份（跳过防抖，用于关键操作如移动/删除）
  async immediateBackup(key, data) {
    // 未登录时跳过服务端备份
    if (!this.isAuthenticated()) {
      if (this.debug) {
        console.log(`📱 Skipping immediate backup for ${key} (user not authenticated)`);
      }
      return;
    }
    ...
    if (this.debug) {
      console.log(`⚡ Immediate backup for key: ${key} (no delay)`);
    }

    // 立即执行备份
    try {
      await this.backupToServer(key, data);
    } catch (error) {
      console.warn(`Immediate backup failed for key ${key}:`, error);
    }
  }
  ...
  getLastSyncTimestamp() {
    if (typeof window === 'undefined') return null;
    return localStorage.getItem('_lastSyncTimestamp');
  }
  ...
  updateLastSyncTimestamp() {
    if (typeof window === 'undefined') return;
    const timestamp = new Date().toISOString();
    localStorage.setItem('_lastSyncTimestamp', timestamp);
    if (this.debug) {
      console.log('⏰ Updated last sync timestamp:', timestamp);
    }
  }
  ...
  clearLastSyncTimestamp() {
    if (typeof window === 'undefined') return;
    localStorage.removeItem('_lastSyncTimestamp');
    if (this.debug) {
      console.log('🗑️ Cleared last sync timestamp');
    }
  }
  ...
  clearAllData() {
    ...
    const keys = [
      ...
      '_lastSyncTimestamp' // ✅ 清空同步时间戳，防止影响新账号的数据合并
    ];

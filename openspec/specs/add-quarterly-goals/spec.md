...
系统 SHALL 提供季度目标数据结构，包含目标的所有必要属性和与年度目标的关联关系。
...
系统 SHALL 以列表形式显示季度目标，支持按年份和季度筛选。

#### Scenario: 显示季度目标列表
- **WHEN** 用户打开季度目标追踪器
- **THEN** 系统 SHALL 显示当前年份和季度的所有季度目标
...
#### Scenario: 显示关联的年度目标
- **WHEN** 季度目标关联了年度目标
- **THEN** 系统 SHALL 在季度目标卡片上显示关联的年度目标标题
- **AND** 系统 SHALL 显示该季度目标的权重占比
- **AND** 点击年度目标标题 SHOULD 跳转到年度目标视图
...
用户 SHALL 能够手动更新季度目标的完成进度，更新后自动触发关联年度目标的进度计算。

#### Scenario: 更新进度百分比
- **WHEN** 用户调整进度滑块或输入进度值（0-100）
- **THEN** 系统 SHALL 更新季度目标的 progress 字段
...
- **AND** 如果关联了年度目标，系统 SHALL 更新年度目标进度
...
系统 SHALL 提供年度目标数据结构，包含目标的所有必要属性，并支持自动进度计算标识。
